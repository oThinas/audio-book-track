...
- `@dnd-kit/core` 6.x + `@dnd-kit/sortable` 8.x + `@dnd-kit/utilities` 3.x (drag-and-drop acessível com `verticalListSortingStrategy` + `KeyboardSensor`) — `chapter.title` (`text NOT NULL`, max 100) + `chapter.position` (`integer NOT NULL`, UNIQUE `(book_id, position)` DEFERRABLE INITIALLY DEFERRED) + `book.chapters_version` (PostgreSQL `integer NOT NULL DEFAULT 0`) (026-chapter-titles-reordering)
...
<!-- SPECKIT START -->
Current plan: [specs/026-chapter-titles-reordering/plan.md](specs/026-chapter-titles-reordering/plan.md)
<!-- SPECKIT END -->
