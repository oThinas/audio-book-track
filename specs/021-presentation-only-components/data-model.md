...
2. **Settings selectors borderline**: `theme-selector` e `font-size-selector` apenas compõem `useTheme` + `useAutoSavePreference`. `useAutoSavePreference` é movido para `src/components/features/settings/hooks/` na Phase 1.5 (T003d) por ser consumido apenas pela feature `settings`. Classificados como 🟢 — não há lógica de domínio no componente, apenas glue. Critério: glue trivial (1 callback de uma linha que delega para hook existente) **não** demanda novo hook.
