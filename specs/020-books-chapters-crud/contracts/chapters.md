...
Atualiza status, narrador, editor ou duração editada de um capítulo. Aplica a máquina de estados (FR-025) e dispara `recomputeBookStatus(book_id)`.
...
z.object({
  ...
  editedSeconds: z.number().int().min(0).max(3_600_000).optional(),
  confirmReversion: z.boolean().optional(),
})
.refine(data => Object.keys(data).length > 0, { message: "Pelo menos um campo deve ser fornecido" });
```

`editedSeconds` é **integer** em segundos. A UI aceita entrada em horas decimais (`"2.5"`) e converte via `Math.round(hours * 3600)` antes de enviar (ex: `2.5 h → 9000 s`).
...
2. Se `currentStatus !== 'paid'`:
   - `status` transição validada via `isValidTransition(current, target, { narratorId, editorId, editedSeconds })`:
     - `pending → editing`: exige `narratorId` não-null (pós-merge com payload).
     - `editing → reviewing`: exige `editorId` não-null e `editedSeconds > 0`.
   ...
   - Campos diretos (`narratorId`, `editorId`, `editedSeconds`) atualizam livremente quando `status` não muda.
...
3. Se reversão `paid → completed` e nenhum outro capítulo `paid` sobra, `book.price_per_hour_cents` passa a ser editável (UI descobre via `GET` subsequente — nenhuma operação explícita aqui).
