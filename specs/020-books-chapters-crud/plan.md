...
**Constraints**: Pagamento (chapter `paid`) imutável exceto via reversão com dupla confirmação; `book.price_per_hour_cents` bloqueado enquanto ≥ 1 capítulo `paid`; soft-delete unificado (nenhuma FK `ON DELETE SET NULL`); livro sempre tem ≥ 1 capítulo (invariante absoluta); preço monetário em `integer` cents (sufixo `_cents`) e duração em `integer` seconds (sufixo `_seconds`), conforme Princípio XI; queries selecionam colunas explícitas (proibido `SELECT *`); toda FK com índice
...
| XI. PostgreSQL | ✅ PASS | `integer` cents para valores monetários (`price_per_hour_cents`, `default_hourly_rate_cents`), `integer` seconds para duração (`edited_seconds`), toda FK com índice, transações nas mutações multi-tabela, `drizzle-kit generate` + `migrate` (sem `push`). |
...
| XIII. Métricas e KPIs | ✅ PASS | Constitution v2.13.0 define o KPI 4 como "Minutagem média por capítulo" = `AVG(chapter.edited_seconds) / 60`. Nenhum campo novo é necessário — `edited_seconds` (já coletado em US5) alimenta o cálculo. Sem deviation. |
...
│   ├── book-edit-increase.spec.ts               # aumentar capítulos em +N, bloqueio de redução, bloqueio de price_per_hour_cents com paid
...
| **#2 — Resolvido pela constitution v2.13.0** | O desvio anterior (Princípio XIII dependia de `num_paginas`) foi resolvido a montante: a constituição define o KPI 4 como "Minutagem média por capítulo" = `AVG(chapter.edited_seconds) / 60`, eliminando a necessidade de um campo dedicado. Nenhum desvio permanece nesta feature em relação ao Princípio XIII. | N/A — deixou de ser desvio. |
