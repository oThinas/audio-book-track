...
- [ ] T010 [US1] Implementar `ListPageLoading` em `src/components/layout/page-loading.tsx`: composição `PageHeader`/`PageTitle`/`PageDescription` + `Button disabled` (ícone `Plus`) + `Input disabled` (ícone `Search`) + `Skeleton` único + `LoadingStatus`, espelhando a moldura de `src/components/features/books/books-client.tsx` (classes/spacing idênticos para CLS zero na moldura)
- [ ] T011 [P] [US1] Criar `src/app/(authenticated)/books/loading.tsx`: `<PageContainer><ListPageLoading …/></PageContainer>` com strings exatas de `books-client.tsx` ("Livros", "Acompanhe capítulos, ganhos e status por livro.", "Novo Livro", "Buscar por título ou estúdio", "Buscar livros")
...
- [ ] T016 [US1] Escrever e validar E2E determinístico em `__tests__/e2e/books-loading-skeleton.spec.ts`: login (helper `login`), `page.route()` atrasando ~1.5s requests com header `RSC: 1` para `/books`, clicar no link da sidebar; asserts: durante o atraso heading "Livros" E `page-loading-skeleton` visíveis; após, skeleton ausente E tabela/empty-state visível (contrato em contracts/loading-states.md); rodar `bun run test:e2e -- books-loading-skeleton`; commit `test: add e2e for books loading skeleton navigation`
