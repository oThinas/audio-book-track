...
export interface DashboardRetrospectiveRouteDeps {
  readonly createService: () => DashboardService;
  readonly today: () => string;
}

const defaultDeps: DashboardRetrospectiveRouteDeps = {
...
export async function handleDashboardRetrospective(
  request: Request,
  _ctx: AuthenticatedContext<Record<string, never>>,
  routeDeps: DashboardRetrospectiveRouteDeps = defaultDeps,
  ...
  const period = parsePeriodSearchParams(url.searchParams, routeDeps.today());
  const widgets = parseWidgetsCsv(url.searchParams.get("widgets"));
  const data = await routeDeps.createService().getRetrospective(period, widgets);
  return NextResponse.json({ data }, { headers: NO_STORE_HEADERS });
}

export const GET = withApiErrorHandler(handleDashboardRetrospective);
