...
async function seedBookWithChapters(opts: {
  studioRepo: InMemoryStudioRepository;
  bookRepo: InMemoryBookRepository;
  chapterRepo: InMemoryChapterRepository;
  statuses: ReadonlyArray<ChapterStatus>;
}) {
  const studio = await opts.studioRepo.create({ name: "Studio", defaultHourlyRateCents: 5000 });
  const book = await opts.bookRepo.insert({
    title: `Book ${crypto.randomUUID().slice(0, 6)}`,
    studioId: studio.id,
...
describe("ChapterService.delete", () => {
  ...
  it("deletes a non-paid chapter and recomputes book.status", async () => {
    const { book, chapters } = await seedBookWithChapters({
      studioRepo,
      bookRepo,
      chapterRepo,
  ...
  it("throws CHAPTER_PAID_LOCKED when the chapter is paid", async () => {
    const { chapters } = await seedBookWithChapters({
      studioRepo,
      bookRepo,
      chapterRepo,
